...
def plot_radar_chart(user_profile, recommendations, numeric_data):
    ...
    categories = ['gender', 'ope', 'con', 'ext', 'agr', 'neu']
...
def plot_scatter_plot(user_profile, recommendations, numeric_data):
    ...
    for idx, (x, y) in enumerate(rec_numeric_data_2d):
        ...
        similarity_score = cosine_similarity([user_profile], [rec_numeric_data.iloc[idx].values])[0][0] * 100
    ...
    for i, row in recommendations.iterrows():
        plt.text(rec_numeric_data_2d[i, 0], rec_numeric_data_2d[i, 1], f"{row['movie']} ({row['year']})", horizontalalignment='left', size='medium', color='black', weight='semibold')
...
if __name__ == '__main__':
    ...
    recommendations_similarity = get_recommendations(user_profile, numeric_data, data, 10, 5.0)
    print("\n\nTop 10 Movie Recommendations (Rating >= 5):")
    print(recommendations_similarity.to_string(index=False, justify='left'))
    plot_radar_chart(user_profile, recommendations_similarity, numeric_data)
    plot_scatter_plot(user_profile, recommendations_similarity, numeric_data)
